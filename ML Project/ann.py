...
for File in os.listdir(direc):  #each file in directory
    ...
    if filename.endswith('.csv'):   #only if csv
        ...
        model = Sequential()
        model.add(Dense(12, input_dim=(36 if dataset.shape[1]==38 else 29), activation='relu')) #add input layer of 36 or 29 nodes and hidden layer of 12 nodes, with relu activation func
