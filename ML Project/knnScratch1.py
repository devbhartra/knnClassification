...
def loadDataset(filename, split, trainingSet=[], testSet=[]):
	with open(filename, 'r') as csvfile:
		...
		for x in range(len(dataset) - 1):
			# print(dataset[x],"\n")
			for y in range(37): #to use all the data values upto the 14th column in the Dataset csv
				dataset[x][y] = float(dataset[x][y])
				# dataset[x][y] = sc_X.fit_transform(dataset[x][y])
...
def main():
	...
	testSet=[]
	split = 0.67
	loadDataset('cat1_test.csv', split, trainingSet, testSet)
